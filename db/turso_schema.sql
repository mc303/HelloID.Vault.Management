-- HR Management System - Turso Database Schema
-- Context7-compatible schema with TEXT-based JSON storage
-- Compatible with Turso (libSQL) cloud database
-- 
-- NOTE: This schema is derived from sqlite_schema.sql with these Turso-specific modifications:
-- - Removed PRAGMA statements (not supported in Turso HTTP API)
-- - Removed FTS5 virtual tables (not supported in Turso)
-- - Removed ANALYZE command (not needed in Turso)
--
-- For local SQLite development, use sqlite_schema.sql instead.
...
CREATE TABLE departments (
    ...
    source TEXT NOT NULL,
    PRIMARY KEY (external_id, source),
    FOREIGN KEY (parent_external_id, source) REFERENCES departments(external_id, source) ON DELETE SET NULL,
...
CREATE TABLE persons (
    ...
    FOREIGN KEY (source) REFERENCES source_system(system_id) ON DELETE SET NULL,
    FOREIGN KEY (primary_manager_person_id) REFERENCES persons(person_id) ON DELETE SET NULL
);
...
CREATE TRIGGER validate_person_custom_fields_json
...
CREATE TRIGGER validate_contract_custom_fields_json
...
CREATE VIEW IF NOT EXISTS person_details_view AS
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                ...
                CASE
                    ...
                    WHEN c.start_date <= date('now') AND (c.end_date > date('now') OR c.end_date IS NULL) THEN 1
                    WHEN c.start_date > date('now') THEN 2
                    WHEN c.end_date < date('now') THEN 3
                    ELSE 4
                END ASC,
                -- 2. Highest FTE
                ...
                COALESCE(c.end_date, '2999-01-01') DESC,
                -- 6. Start date ascending
                COALESCE(c.start_date, '9999-12-31') ASC,
...
SELECT
    ...
    CASE
        ...
        WHEN pc.start_date > date('now') THEN 'Future'
        WHEN pc.end_date IS NOT NULL AND pc.end_date < date('now') THEN 'Past'
        ELSE 'Active'
        ...
        WHEN pc.start_date IS NOT NULL THEN
            CASE
                WHEN pc.end_date IS NOT NULL THEN
                    strftime('%Y-%m-%d', pc.start_date) || ' until ' || strftime('%Y-%m-%d', pc.end_date)
                ELSE
                    strftime('%Y-%m-%d', pc.start_date) || ' until ongoing'
            END
        ELSE NULL
...
SELECT
    ...
    CASE
        ...
        WHEN c.start_date > date('now') THEN 'Future'
        WHEN c.end_date IS NOT NULL AND c.end_date < date('now') THEN 'Past'
        ELSE 'Active'
        ...
        WHEN c.start_date IS NOT NULL THEN
            CASE
                WHEN c.end_date IS NOT NULL THEN
                    strftime('%Y-%m-%d', c.start_date) || ' until ' || strftime('%Y-%m-%d', c.end_date)
                ELSE
                    strftime('%Y-%m-%d', c.start_date) || ' until ongoing'
            END
        ELSE NULL
...
INSERT INTO primary_contract_config (field_name, display_name, sort_order, priority_order, is_active) VALUES
('fte', 'FTE (Full-Time Equivalent)', 'DESC', 1, 1),
('hours_per_week', 'Hours per Week', 'DESC', 2, 1),
('sequence', 'Sequence', 'DESC', 3, 1),
('end_date', 'End Date', 'DESC', 4, 1),
('start_date', 'Start Date', 'ASC', 5, 1);

-- AD Correlation Configuration (single row, id = 1)
...
-- AD Correlation Results (one row per person)
...
-- AD Match Recommendations (scored suggestions for uncorrelated persons)
...
CREATE VIEW IF NOT EXISTS contract_json_view AS
SELECT
    -- Contract fields
    c.contract_id,
    c.external_id,
...
CREATE VIEW IF NOT EXISTS orphaned_references_report AS
SELECT
    ...
    GROUP_CONCAT(DISTINCT c.external_id) AS sample_contracts
FROM contracts c
LEFT JOIN source_system ss ON c.source = ss.system_id
...
GROUP BY c.department_external_id, c.source
...
GROUP BY c.location_external_id, c.source
...
GROUP BY c.cost_center_external_id, c.source
...
GROUP BY c.cost_bearer_external_id, c.source
...
GROUP BY c.employer_external_id, c.source
...
GROUP BY c.team_external_id, c.source
...
GROUP BY c.division_external_id, c.source
...
GROUP BY c.title_external_id, c.source
...
GROUP BY c.organization_external_id, c.source
...
CREATE VIEW IF NOT EXISTS source_distribution_metrics AS
SELECT
    ss.system_id,
    ss.display_name AS source_name,
...
CREATE VIEW IF NOT EXISTS cross_source_violations AS
SELECT
...
INSERT OR IGNORE INTO cache_metadata (cache_name, row_count)
...
-- Compound index for common status + date range filters (replaces 7 single-column indexes)
CREATE INDEX IF NOT EXISTS idx_contract_cache_status_dates ON contract_details_cache(contract_status, start_date, end_date);
...
CREATE VIEW IF NOT EXISTS person_list_view AS
SELECT
    p.person_id,
    p.display_name,
    p.external_id,
    ...
    CASE
        ...
        WHEN EXISTS (
            ...
            AND (c.end_date > date('now') OR c.end_date IS NULL)
        ) THEN 'Active'
...
CREATE VIEW IF NOT EXISTS source_systems_view AS
SELECT
    ss.system_id,
    ss.display_name,
    ss.identification_key,
